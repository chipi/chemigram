...
The **masking component** (subsystem 4) is `chemigram.core.masking.dt_serialize` plus the `apply_with_mask` helper in `chemigram.core.helpers` (formerly `apply_with_drawn_mask`; backcompat alias preserved). v1.4.0 introduced drawn-form serialization (path 4a); v1.5.0 retired the earlier PNG-based architecture entirely (ADR-076 supersedes ADR-021/022/055/057/058/074). v1.9.0 closed the mask + retouch architecture trilogy: spatial masks via `dt_form` (gradient / ellipse / rectangle / path; RFC-029 / ADR-084), parametric range filters via `range_filter` (luminance + HSL color; RFC-024 / ADR-085), retouch heal/clone via the `apply_spot` MCP tool (RFC-025 / ADR-087), and content-derived masks via the chat-client's vision-capable LLM (RFC-026 / ADR-086). The wire format is darktable's native byte schema throughout — geometric forms encoded into `<darktable:masks_history>`, parametric ranges via `blendif` bytes, retouch via the 13260-byte `dt_iop_retouch_params_t` struct. No PNG, no per-image registry. The deployed sibling-provider precision tier (RFC-030, drafted, deferred) lands when LLM-vision precision becomes the bottleneck on real workflows.
