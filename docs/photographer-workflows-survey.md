...
- **Mapping:** ✅ **shipped in v1.10.0 as `propagate_state` (RFC-037 / ADR-090).** Closes the gap at full LR-Sync parity. ADR-090 ships as Draft pending darkroom-session validation against a real wedding burst.
...
| Gap | Severity | Photographers reaching for it | RFC / vocabulary candidate |
|-|-|-|-|
| **Batch-sync workflow / "edit one image, propagate to N"** | **High** (4/6 — load-bearing for the genre) | W2 Flemming, W4 Hunter & Sarah, W5 Davidson, W6 SLR Lounge | ✅ **Shipped in v1.10.0 as RFC-037 / ADR-090** (`propagate_state`). LR-Sync-parity: inherit-everything-by-default with framing-bound auto-exclusion (`ashift` / `crop` / `retouch` / `lens` / drawn-mask-bound). Atomic; cap 200 targets per call. ADR-090 ships as Draft pending darkroom-session validation against a real wedding burst. |
...
**Implication for chemigram.** Closed by RFC-031 (`apply_per_region`, shipped this round) for the single-primitive case, then ✅ **un-deferred and shipped as RFC-036 / ADR-089 in v1.10.0** for the mixed-op case (the B&W finding that "the same region often needs both contrast AND color sculpting in one move" was load-bearing for the un-deferral). Each region in mixed-op shape carries its own `ops: [{entry, parameter_values?}, ...]`; per-(op, region) `multi_priority` allocation; cap 64 (op × region) pairs. Thompson's *low-magnitude high-iteration* discipline (1-3% per layer over weeks) is now expressible as repeated `apply_per_region` calls or as a single mixed-op call carrying all layered ops. Visual review (debt tracker item 8) should validate this works in practice.
...
**Implication for chemigram.** ✅ **Closed in v1.10.0** — RFC-037 / ADR-090 shipped `propagate_state(source_workspace, target_workspaces, exclude_ops?, include_per_image?, label?)`. Reframed during deliberation from predefined scopes (`"wb-only"`, `"exposure-only"`) to **inherit-everything-by-default with framing-bound auto-exclusion** — same discipline as Lightroom Sync. The scope-preset enumeration would have been leaky; LR-parity is cleaner. Atomic-batch semantics; cap 200 targets per call. ADR ships as Draft pending darkroom-session validation against a real wedding burst.
