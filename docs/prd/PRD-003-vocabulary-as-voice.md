...
A year into using Chemigram, the photographer opens `~/Pictures/Chemigram/vocabulary/personal/manifest.json` and scrolls. There are 187 entries. `wb_pelagic_recovery_subtle`. `tone_lifted_shadows_subject`. `fuji_acros_red_filter_personal`. `clarity_rock_texture_session_dry`. Each one was authored on a specific evening, for a specific image, when the agent didn't have a word for what was wanted. Some were authored once and used hundreds of times. Some were authored once and never used again. Reading the list is unexpectedly moving — it's a self-portrait. Not "what I look like" but "how I edit." The names alone tell a story: the photographer who keeps reaching for *subject* and *recovery* and *rock texture* is someone with a body of work centered on living things in difficult light. They didn't choose those words abstractly; they accumulated them by editing. The vocabulary is the photographer's voice, made visible.
...
In a session, the agent reaches for `tone_lifted_shadows_subject` (starter) and applies it. The photographer says, "yes, but a little less aggressive on the deep shadows." The agent doesn't have an entry for that exact intention. It improvises (mask + lower-intensity tone primitive). It logs the gap. The session continues.

Later — that night, or next weekend, or in a focused vocabulary-authoring evening — the photographer opens darktable, sets up the move they want (lifted shadows, dialed-back deep-shadow zone, restricted to subject), captures it as a `.dtstyle` named `tone_lifted_shadows_subject_subtle`. Adds a manifest entry: layer L3, subtype look, touches `toneequalizer`, mask_kind raster (referencing `current_subject_mask`), description "lift shadows on subject, avoiding deepest zones for natural feel." Drops both files into their personal vocabulary. From the next session on, this primitive is part of the agent's action space.
...
2. **MCP enables vocabulary use.** Without an agent that can call vocabulary as named tools (`apply_primitive("tone_lifted_shadows_subject")`), the vocabulary is just a file collection. MCP turns it into an action space.
