...
It's just past 11 PM. The iguana raw is open. You haven't touched it in three weeks; tonight you're going to finish it. You drop the brief into the conversation: "the iguana on the rock, late afternoon. I want the subject to feel present without being pushed. Subtle." The agent reads, takes a moment, comes back: "I see the image. Looking at your taste, you reach for `radial_subject_lift` for this kind of subject-ground relationship — drops a centered ellipse on the iguana and lifts +0.6 EV through it. Should I start there? I notice the highlights are clipping a bit on the rock above; we might also want `gradient_top_dampen_highlights` to roll those back." You say yes to the first, defer the second. Render. The lift takes; the iguana steps forward against the rock. You snapshot, branch to "v1," try a tighter custom variant of the radial, judge it as too much, abandon the branch, return to v1 and refine. Twenty minutes pass. The image is done. The agent asks: "I noticed you reach for the radial-subject-lift pattern often when working with a clear subject-ground relationship — should that pattern be in your taste?" You say yes. The session closes; `taste.md` grows by one line. Next time, the agent will already know.
...
Each move is a vocabulary application. `apply_primitive("radial_subject_lift")` takes a known shape — the agent knows it touches `exposure`, bound to a centered ellipse drawn into the XMP's `masks_history` (per ADR-076; the engine routes through the drawn-mask apply path automatically when the entry's `mask_spec` is set). The render comes back in 2 seconds. The photographer judges: "yes, that's the move." Or: "too much." The agent adjusts: "I can either dial back to `radial_subject_lift_subtle` if you've authored that personal variant, or undo and try `gradient_bottom_lift_shadows` to lift the foreground without the radial restriction." The photographer chooses; the agent applies; the loop continues.
...
- **Pixel-precise content-aware masking by default.** v1.5.0 ships drawn-form geometric masks (gradient / ellipse / rectangle) baked into vocabulary entries' `mask_spec`. Subject-precise content-aware masking ("mask the iguana exactly") arrives in Phase 4 via a sibling project (`chemigram-masker-sam`) producing darktable drawn-form geometry. The PNG-mask path that earlier drafts of this PRD assumed turned out to be a silent no-op and was retired in v1.5.0 (ADR-076 supersedes RFC-004).
