...
| **Phase 1.2** | Vocabulary expansion — Path B unblock + 35 new entries (`expressive-baseline` pack) calibrated to the taste-library artist profiles | **In flight (v1.2.0)** — slice-and-gate. RFC-018 v0.2 argues the simplified Path B implementation + entry table (per-entry iop_order proven unnecessary in dt 5.4.1, see `tests/fixtures/preflight-evidence/`); closes into ADR-063 (Path B unblocking) + ADR-064 (authoring workflow). Engine foundation shipped (#41/#42/#44); cb-axis pre-flight + 35-entry authoring + Mode A prompt v3 remain. |
