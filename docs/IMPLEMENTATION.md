...
| **Phase 1.3** | Command-line interface | **In flight (v1.3.0)** — slice-and-gate. RFC-020 + PRD-005 land the design. Adds `chemigram.cli` subprocess-callable adapter mirroring the MCP tool surface verb-for-verb (per ADR-033/056). Output: human default + NDJSON via `--json`. Closes into ADR-069/070/071/072 at ship. |
| **Phase 1.4** | `expressive-baseline` vocabulary authoring | **Hands-on darktable work, post-v1.3.0.** 35 entries (16 Path A + 19 Path B) calibrated to the taste-library artist profiles. Engine and assertion library are already in v1.2.0; this phase is purely authoring + e2e direction-of-change tests. Issues #40/#45/#46/#47 move here from v1.2.0. Ships as v1.4.0. |
| **Phase 2** | Vocabulary maturation — grow vocab from session evidence | **Begins post-v1.4.0** — use-driven; intermittent; grows the *personal* pack on top of starter + expressive-baseline. Not slice-and-gate. |
