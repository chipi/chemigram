...
| **Phase 1.2** | Vocabulary expansion + reference-image validation — Path B unblock, 35 new entries (`expressive-baseline` pack), and a CI-safe colorimetric assertion library | **In flight (v1.2.0)** — slice-and-gate. RFC-018 v0.2 argues the simplified Path B implementation + entry table (per-entry iop_order proven unnecessary in dt 5.4.1, see `tests/fixtures/preflight-evidence/`); closes into ADR-063 (Path B unblocking) + ADR-064 (authoring workflow). RFC-019 v0.2 argues the synthetic-only reference fixture path; closes into ADR-066 (fixture policy) + ADR-067 (assertion protocol) + ADR-068 (darktable version gate, deferred). Engine foundation, assertion library, and synthetic Tier A shipped (#41/#42/#44 + `chemigram.core.assertions` + `tests/fixtures/reference-targets/`); cb-axis pre-flight + 35-entry authoring + Mode A prompt v3 remain. |
| **Phase 2** | Vocabulary maturation — grow vocab from session evidence | **Begins post-v1.2.0** — use-driven; intermittent; grows the *personal* pack on top of starter + expressive-baseline. Not slice-and-gate. |
