...
| **Phase 1.2** | Vocabulary expansion — Path B unblock + 35 new entries (`expressive-baseline` pack) calibrated to the taste-library artist profiles | **Planned (v1.2.0)** — slice-and-gate. RFC-018 argues the iop_order strategy + entry table; closes into ADR-063 (iop_order) + ADR-064 (authoring workflow). Pre-flight experiments precede authoring. |
