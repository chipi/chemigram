...
- **Phase 1** (minimum viable loop) — **in progress**. Slices 1–4 shipped (v0.1.0–v0.4.0). v0.1.0: dtstyle parser, XMP r/w + synthesizer, render pipeline, EXIF binding. v0.2.0: `chemigram.core.versioning` package (canonical_bytes/xmp_hash, ImageRepo, snapshot/checkout/branch/log/diff/tag, mask registry). v0.3.0: `chemigram.core.vocab`, `chemigram.mcp.prompts`, `chemigram.mcp.server` framework, 27 MCP tools, `chemigram.core.workspace` orchestrator. v0.4.0: `chemigram.core.masking` (MaskingProvider Protocol + CoarseAgentProvider sampling-based default), real generate_mask/regenerate_mask MCP tools, mask-bound L3 vocabulary entries with `apply_primitive(mask_override=...)` real path. 10 of 17 RFCs closed: RFC-001 → ADR-050, RFC-002 → ADR-054, RFC-003 → ADR-055, RFC-004 → ADR-058, RFC-005 → ADR-052, RFC-006 → ADR-051, RFC-009 → ADR-057, RFC-010 → ADR-056, RFC-015 → ADR-053, RFC-016 → ADR-043/044/045. Slice 5 (context layer) ships next as v0.5.0.

If you're doing implementation work, you're at the end of Phase 1 Slice 4 (masking complete; mask-bound primitives end-to-end). Slice 5 (context layer) is up next — it fills the 5 context-stub tools from v0.3.0 with real readers/writers of `~/.chemigram/tastes/`, `brief.md`, `notes.md`, plus session transcripts. The slices are described in `IMPLEMENTATION.md`'s Phase 1 section.
