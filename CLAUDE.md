...
- **Phase 1** (minimum viable loop) — **in progress**. Slice 1 (v0.1.0) shipped: parser, XMP r/w + synthesizer, render pipeline, EXIF binding. Slice 2 (v0.2.0) shipped: `chemigram.core.versioning` package with `canonical_bytes`/`xmp_hash`, per-image `ImageRepo` (objects/refs/HEAD/log), high-level `snapshot`/`checkout`/`branch`/`log`/`diff`/`tag`, mask registry. 6 of 17 RFCs closed: RFC-001 → ADR-050, RFC-002 → ADR-054, RFC-003 → ADR-055, RFC-005 → ADR-052, RFC-006 → ADR-051, RFC-015 → ADR-053. Slice 3 (MCP server + prompt system) is next.

If you're doing implementation work, you're at the end of Phase 1 Slice 2 (versioning + masks complete; gate exercised in Slice 6). Slice 3 (MCP server + prompt system) is next. The slices are described in `IMPLEMENTATION.md`'s Phase 1 section (Slices 1 through 6, each with its scope, gate, and the RFCs that close at that gate).
